...
//
// The eligibility rule is not cosmetic and is worth stating. `m.rated` is how this excludes
// tables a house bot had to fill: crediting a model for beating an engine bot would put a
// fictional opponent into the likelihood.
//
// This was one of two boards. The platform harness board ran the same estimator over the
// platform's own agents and has been removed along with the rest of the lab benchmark; its
// eligibility guard was structural (every seat had to be a harness agent) because its matches
// were unrated by design and could not use the filter above. This board keeps the stronger
// guard, and is now the only one.
