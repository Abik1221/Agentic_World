...
// UpsertAppleAccount implements identity.Repo. Deliberately mirrors
// UpsertGitHubAccount step-for-step, keyed on apple_sub instead of github_id.
func (r *IdentityRepo) UpsertAppleAccount(ctx context.Context, in identity.AppleUpsertInput) (identity.AppleUpsertResult, error) {
	// 1. Already linked to this Apple identity → log in.
	var uPub, aPub, aName string
	err := r.db.QueryRow(ctx,
		`SELECT u.public_id, a.public_id, a.name
		 FROM users u JOIN agents a ON a.owner_user_id = u.id
		 WHERE u.apple_sub = $1
		 ORDER BY a.id LIMIT 1`, in.AppleSub).Scan(&uPub, &aPub, &aName)
	if err == nil {
		return identity.AppleUpsertResult{UserPublicID: uPub, AgentPublicID: aPub, AgentName: aName}, nil
	}
	if !errors.Is(err, pgx.ErrNoRows) {
		return identity.AppleUpsertResult{}, err
	}

	// 2. A pre-existing, unlinked email account (Apple verified this email, including
	// Hide My Email relays) → link it.
	if in.Email != "" {
		var uid int64
		e2 := r.db.QueryRow(ctx,
			`SELECT u.id, u.public_id, COALESCE(a.public_id,''), COALESCE(a.name,'')
			 FROM users u LEFT JOIN agents a ON a.owner_user_id = u.id
			 WHERE u.email = $1 AND u.apple_sub IS NULL
			 ORDER BY a.id LIMIT 1`, in.Email).Scan(&uid, &uPub, &aPub, &aName)
		if e2 == nil {
			if _, err := r.db.Exec(ctx,
				`UPDATE users SET apple_sub = $1 WHERE id = $2 AND apple_sub IS NULL`, in.AppleSub, uid); err != nil {
				return identity.AppleUpsertResult{}, err
			}
			return identity.AppleUpsertResult{UserPublicID: uPub, AgentPublicID: aPub, AgentName: aName}, nil
		}
		if !errors.Is(e2, pgx.ErrNoRows) {
			return identity.AppleUpsertResult{}, e2
		}
	}
	...
	var userID int64
	err = tx.QueryRow(ctx,
		`INSERT INTO users (public_id, email, apple_sub) VALUES ($1, $2, $3) RETURNING id`,
		in.UserPublicID, nullString(in.Email), in.AppleSub).Scan(&userID)
	if err != nil {
		if isUniqueViolation(err) {
			return identity.AppleUpsertResult{}, identity.ErrEmailTaken
		}
		return identity.AppleUpsertResult{}, err
	}
	if _, err = tx.Exec(ctx,
	...
	return identity.AppleUpsertResult{
		UserPublicID: in.UserPublicID, AgentPublicID: in.AgentPublicID, AgentName: in.AgentName, Created: true,
	}, nil
}
