...
func TestDecide_ClassifiesOutcomes(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			d := newDriver(tc.fn)
			card, out, latency, _, _ := d.decide(context.Background(), socketSeat{gw: d.gw, id: "ag1", matchID: "m1"}, 0, "ag_bench", "m1", viewWithHand(hand))
			if card != tc.wantCard {
...
func TestDecide_FeedsRecorder(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		_, out, ms, _, _ := legal.decide(context.Background(), socketSeat{gw: legal.gw, id: "ag0", matchID: "m1"}, 0, "ag_bench", "m1", viewWithHand(hand))
		rec.Record(benchmark.Decision{Seat: 0, AgentID: "ag0", Outcome: out, LatencyMS: ms})
	}
	_, out, ms, _, _ := timeout.decide(context.Background(), socketSeat{gw: timeout.gw, id: "ag0", matchID: "m1"}, 0, "ag_bench", "m1", viewWithHand(hand))
	rec.Record(benchmark.Decision{Seat: 0, AgentID: "ag0", Outcome: out, LatencyMS: ms})
