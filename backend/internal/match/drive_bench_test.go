...
func TestDecide_ClassifiesOutcomes(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			d := newDriver(tc.fn)
			card, out, latency := d.decide(context.Background(), "ag1", "m1", viewWithHand(hand))
			if card != tc.wantCard {
...
func TestDecide_FeedsRecorder(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		_, out, ms := legal.decide(context.Background(), "ag0", "m1", viewWithHand(hand))
		rec.Record(benchmark.Decision{Seat: 0, AgentID: "ag0", Outcome: out, LatencyMS: ms})
	}
	_, out, ms := timeout.decide(context.Background(), "ag0", "m1", viewWithHand(hand))
	rec.Record(benchmark.Decision{Seat: 0, AgentID: "ag0", Outcome: out, LatencyMS: ms})
